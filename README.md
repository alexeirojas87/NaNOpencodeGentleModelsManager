# Model Dashboard

A local, single-user web dashboard for editing OpenCode provider/model
configuration and gentle-ai orchestration model assignments in
`~/.config/opencode/opencode.json` (called **CONFIG** below). It is a
surgical config editor, not a generator: it loads the real file, patches
only an allowlist of key paths, validates before anything is written, keeps
timestamped restore points, and replaces the file atomically. Removing the
dashboard leaves CONFIG untouched.

## Quick start

Prerequisites: Node ≥ 22.22.2 and pnpm 11 (pinned via `packageManager`).

```bash
pnpm install
pnpm start            # one command: build + serve on http://127.0.0.1:8787
```

`pnpm start` builds the web bundle and serves UI and API from the same
process — open the printed URL. If port 8787 is already held on your
machine, pick another for the run: `PORT=8791 pnpm start` (the port only
needs to be free; production UI and API are same-origin). A collision
prints one actionable line and exits.

For development with hot reload use `pnpm dev` (Vite on
`http://localhost:5173` proxying `/api` to `127.0.0.1:8787` — that pairing
does require 8787 to be free).

| Command             | What it does                                                          |
| ------------------- | --------------------------------------------------------------------- |
| `pnpm start`        | build + single-process server on `127.0.0.1:8787` (or `PORT=<n>`)     |
| `pnpm dev`          | tsx-watch server + Vite dev server (proxies `/api`)                   |
| `pnpm build`        | Production bundle to `dist/` (the server then serves it with the API) |
| `pnpm test`         | vitest full suite with coverage (node + jsdom projects)               |
| `pnpm typecheck`    | `tsc --noEmit`                                                        |
| `pnpm lint`         | `eslint .`                                                            |
| `pnpm format:check` | `prettier --check .`                                                  |

## What the dashboard manages

Everything is derived from CONFIG's real keys — provider ids, model ids and
agent names are data, never hardcoded constants, so anything added or
removed externally (including by `gentle-ai sync`) shows up on reload.

| View          | Manages                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Providers     | `provider.<id>.name`, `.npm`, `.options.baseURL`; `.options.apiKey` replace-only, never displayed                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Models        | Whole `provider.<id>.models.<model-id>` entries — add (optionally pre-filled from the bundled NaN snapshot), edit any field, remove behind an explicit confirm; declared-vs-snapshot drift shown as advisory chips                                                                                                                                                                                                                                                                                                 |
| Orchestration | `agent.<name>.model` per agent (set a `provider/model` pair or clear to runtime default); create new agents from a bundled template or a clone of an existing agent; **pipeline builder** (agent-pipelines): create/edit an N-role pipeline atomically — see §Agent pipelines; prompt bodies and `gentle-ai:*` markers render read-only in a reader modal (user-owned prompts editable via the gated editor); default-agent picker offers visible primaries only; "Run sync" executes `gentle-ai sync` server-side |
| Status        | Read-only: CONFIG path/hash/mtime, drift rollup, backup restore points, restart flag                                                                                                                                                                                                                                                                                                                                                                                                                               |

Writes flow through `PUT`/`POST`/`DELETE /api/*` only; the browser never
touches the filesystem, and every write must echo the hash the last read
reported (see safety model).

## Agent pipelines

A **pipeline** is one orchestrator agent plus N hidden role subagents created
as a unit from the Orchestration view. Definitions (generator inputs: role
names, models, descriptions, resolved prompt text, prompt source, helper
names) live under the top-level `agent-pipelines` key in CONFIG — an
editing/provenance truth; the materialized `agent.*` rows are the runtime
truth. The key is inert to OpenCode's loader and survives OpenCode's own
config rewrites and `gentle-ai sync` deep-merges byte-identically (proven by
`test/harness/` — see §Sandbox rule).

- **One save, zero mutation on any gate failure**: create/edit/delete writes
  the definition + orchestrator row + every role row through a single
  validate→backup→atomic-replace pass; duplicates, collisions, reserved-name
  claims and shape violations are rejected before anything touches disk.
- **Generated fields are server-side only**: `mode` (`primary`/`subagent`),
  `hidden` (roles never appear in pickers), the orchestrator's delegation
  prompt and its `permission.task` map (`"*":"deny"` first, then exact-name
  allows — last-match-wins order is load-bearing) are derived from the role
  set; client input carrying them is rejected. `temperature`/`variant` are
  never invented; an absent role `model` inherits the invoker's model.
- **The builder is the edit path**: generic `agent.*` mutations on member
  names are rejected naming the owning pipeline; prompt updates on a role
  rewrite definition and row in lockstep; deletes are pipeline-level (zero
  orphan rows either direction) with backup-before-delete.

## Safety model

| Guarantee                                     | Mechanism                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Key-path allowlist                            | Only `provider.<id>.{name,npm,options.baseURL,options.apiKey}`, `provider.<id>.models.<model-id>` (whole entries), `agent.<name>.{model,description,temperature,variant}` for NON-pipeline-owned names, `default_agent` (gated to existing visible non-subagent primaries), `agent.<name>` whole-entry create, plus the four pipeline widenings: `agent-pipelines` whole-value writes (strict shape, gates per referenced name), `agent.<name>.prompt` updates (user-owned names only, inline ≤ 64 KiB, `{file:…}` refs rejected whole-string), and `agent.<name>` whole-entry deletes (user-owned standalone names only) — everything else (`$schema`, `compaction`, `instructions`, `mcp`, `share`, unknown keys, reserved-name prompts, `gentle-ai:*` markers) is rejected before apply, naming the offending path (HTTP 400) |
| Stale-write rejection                         | Each read echoes a SHA-256 of the exact bytes; each write carries it back. Mismatch → 409 before any file mutation, before any backup, with pending edits kept for reload-and-reapply                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Validate before backup                        | Patched trees are schema-validated (offline zod schema generated from `opencode.ai/config.json`, permissive: unknown keys never rejected). Invalid edits produce readable per-path errors and no backup or write happens                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Per-save backup + atomic replace              | Every accepted save first copies the current file to `model-dashboard-backups/opencode-<ISO>.json` beside CONFIG (newest 20 kept, modes preserved), then writes a hidden same-directory tmp file, fsyncs, chmods to the original mode, and `rename()`s it over CONFIG — readers never observe a truncated file                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Byte-exact invariant                          | Saves serialize `JSON.stringify(tree, null, 2)` in insertion order (no sorting, no trailing newline). A no-op save leaves the file byte-identical; untouched subtrees keep their exact bytes and key order                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| apiKey confinement                            | `options.apiKey` values never leave the server: API responses carry only `{ "configured": true                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | false }`, the UI never renders or stores a key, and masked shapes are never echoed back as if they were values. The field is replace-only — empty input means "keep the bytes" |
| Localhost-only, offline, no telemetry         | The server binds `127.0.0.1:8787` with no auth (single-user machine). The schema and the NaN catalog snapshot ship inside the app; the dashboard makes zero outbound network calls                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Sync is a pass-through, not a command surface | `gentle-ai sync` runs via `execFile` (shell never involved) with a fixed `['sync', ...]` argv prefix; only `--profile`, `--profile-phase` and `--sdd-profile-strategy` are accepted, each with a validated value shape; anything else is a 400 before spawn. 60 s timeout, 1 MB output cap, exit code and output surfaced honestly (non-zero never renders a success state)                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Restart notice                                | CONFIG has no hot-reload contract: after any successful save the UI shows "Restart OpenCode to apply", and `GET /api/status` reports the same flag process-wide                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Agent create is set-only, edits are gated     | `POST /api/agents` creates `agent.<name>` as one whole-entry set through the same validate→backup→atomic-replace pipeline; an existing name is rejected `agent_exists` (400) **before any mutation**. After creation only two paths move: the gated `agent.<name>.prompt` update (user-owned names, reserved rejected; pipeline roles update their definition in lockstep) and the user-owned standalone delete with backup (see the allowlist row and §Agent pipelines)                                                                                                                                                                                                                                                                                                                                                         |
| Create gates run in a pinned order            | Name syntax (`^[\w.-]+$`, dot-segments and prototype keys rejected), the gentle-ai reserved-name blocklist (`sdd-`/`jd-`/`review-` prefixes; exact `general`, `explore`, `gentle-orchestrator`), an entry-key whitelist ({model?, description?, prompt}), prompt literals only (inline, ≤ 64 KiB, `{file:…}` refs rejected — clones materialize them server-side), and the installed-pair model check all return a naming 400 with zero file mutation; a stale hash still surfaces the usual 409 conflict with pending form data kept                                                                                                                                                                                                                                                                                            |

## Two backup systems — don't mix them up

| Directory                                     | Written by                                      | Contains                                                                                            | How to restore                                                                                                                                                             |
| --------------------------------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `~/.config/opencode/model-dashboard-backups/` | This dashboard, once per accepted save          | Pre-save snapshots of CONFIG (newest 20; `CONFIG_PATH` relocates them beside the file it points at) | Stop OpenCode, copy a backup over CONFIG (`cp "<backup>" "<config path>"` — the Status view shows the newest, ready to copy), restart. The dashboard itself never restores |
| `~/.gentle-ai/backups/`                       | The `gentle-ai` CLI, around its own sync writes | Pre-sync states                                                                                     | `gentle-ai restore` — owned by gentle-ai, not by this app; the two directories are independent                                                                             |

## Sandbox rule (how tests stay safe)

Automated tests **never touch the live `~/.config`**. The fs boundary is
redirected by environment:

- `CONFIG_PATH` — every filesystem-touching unit/integration test points it
  at a temp-directory copy of `test/fixtures/config.sample.json` (a fully
  synthetic fixture). The real CONFIG is never read or written by tests.
- `AUTH_PATH` — auth.json is referenced by existence only, and always
  sandboxed; secret values are never loaded into responses.
- `GENTLE_AI_BIN` — subprocess tests run a local stub binary instead of
  `gentle-ai`; no automated test ever executes the real CLI. Real-binary
  verification runs manually against a sandboxed `HOME` holding copies,
  with live file mtimes verified unchanged afterwards.
- `OPENCODE_BIN` / `RUN_OPENCODE_HARNESS` — the standalone sandbox harnesses
  (`test/harness/*.mjs`, run with `node`, outside vitest):
  - `node test/harness/sync-pipeline.mjs` — AP-8: dashboard-created pipeline
    through a REAL `gentle-ai sync` under a mkdtemp HOME; definition and
    every row golden byte-identical post-sync.
  - `node test/harness/opencode-boot-inert.mjs` — AP-1/C-R2: the real
    opencode binary boots with `agent-pipelines` present and its own config
    rewrite preserves the key.
  - `RUN_OPENCODE_HARNESS=1 node test/harness/runtime-parity.mjs` — opt-in
    AP-3/AP-7 live probes (agent selection, role absence, delegation-map
    order); exits 0 with a SKIP line when unset or when binaries are missing
    (skip-if-missing pattern). Pure-TUI checks (Tab cycle, `@` autocomplete,
    credential-bound delegation) live in `test/harness/CHECKLIST.md`.
    All three spawn only against the sandbox HOME and prove the live
    `~/.config` sha/mtime unchanged before-and-after.
- Web tests script `fetch` (jsdom + @testing-library/react) — no network,
  no filesystem.

## Theme and accessibility

The console ships a light indigo theme (orchestration-v2 WU-C). Every color,
radius and shadow is a single-source `:root` custom property in
`web/src/styles.css` — no view or component hardcodes one — so retheming is
one knob: change a token value and every surface follows.

- **WCAG 2.1 AA, machine-checked:** `web/src/contrast.test.ts` parses the
  stylesheet's `:root` block and asserts the pinned token-pair table at
  ≥ 4.5:1 for text and ≥ 3:1 for UI parts and focus rings. Change a token
  and `pnpm test contrast` re-proves (or breaks) accessibility — pairs are
  pinned by role, not by literal hex.
- **Roles:** indigo `--accent` carries focus rings and UI parts;
  `--accent-strong`/`--accent-active` carry links, the active-nav pill and
  primary buttons; the amber family survives only as `--warning` (drift
  chips, restart banners, validation errors).
- **Motion & focus:** the `:focus-visible` ring and the
  `prefers-reduced-motion` kill-block are preserved; the only animations
  are the save-bar transition and the bundled spinner.
- **Icons:** 14 hand-drawn inline SVGs in `web/src/icons.tsx`
  (`currentColor` strokes, pastel tile backgrounds from tokens) — zero new
  dependencies, and the offline build stays green.

## Rollback

- **Code**: one revertable Conventional commit per work unit, stacked on
  `main`. Revert newest-first; earlier units don't depend on later ones.
  The stack so far: `757e8f6` (bootstrap) → `8f836a8` (load/mask) →
  `f25628f` (harness fix) → `48025cb` (schema/patch/validate) →
  `6885408` (save pipeline) → `19c4fb3` (REST API) → `52e7c78` (Providers
  view) → `51070b3` (models/snapshot/drift) → `b1d845c` (Orchestration
  view) → `f3ed949` (sync pass-through) → this unit (Status view + polish
  - README; see `git log` for its sha).
- **Config data**: restore the newest `model-dashboard-backups` file (or
  any listed one) with `cp` as described above; use `gentle-ai restore`
  for changes made by `gentle-ai sync`. A 409 rejection path means a
  concurrent external editor can never be clobbered silently.
- **Full uninstall**: delete the app and its backup directory; CONFIG
  itself was never owned by the dashboard and stays in place.

## Layout

```text
server/src/    Hono app: config core (load/mask/patch/validate/save/backup),
               REST routes (/api/*), sync pass-through, bundled NaN snapshot
web/src/       Vite/React console: rail + Providers/Models/Orchestration/Status
               views, api.ts (only network surface), components, styles
shared/        DTOs (shared/types.ts) + committed generated zod schema
tools/         gen-schema.ts — one-time offline schema regeneration
test/          fixtures, unit (node) and integration (app.request()) suites
test/harness/  standalone sandbox harnesses (sync golden, boot-inert, opt-in
               runtime parity) — node .mjs scripts outside vitest, real
               binaries under mkdtemp HOME only, plus CHECKLIST.md
```
