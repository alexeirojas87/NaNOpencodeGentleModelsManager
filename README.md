...
| Orchestration | `agent.<name>.model` per agent (set a `provider/model` pair or clear to runtime default); create new agents from a bundled template or a clone of an existing agent (prompt fixed at creation); prompt bodies and `gentle-ai:*` markers render read-only in a reader modal; "Run sync" executes `gentle-ai sync` server-side |
